...
include.module( 'tool-mapimageexport',  [ 'tool', 'widgets', 'tool-mapimageexport.panel-mapimageexport-html', 'proj4' ], function ( inc ) {
    ...
    function getScale(){
        let zoom = SMK.MAP[1].viewer.type == "esri3d" ? Math.round(SMK.MAP[1].$viewer.view.zoom) 
        		                                      : Math.round(SMK.MAP[1].$viewer.map.getZoom());
        let scale; // = SMK.UTIL.calculateZoomScale(zoom);
        
        if (zoom > 1)
        {
            //I've been finding the exact zoom level is usually closer than you would expect
            zoom = SMK.MAP[1].viewer.type == "esri3d" ? zoom + 1 : zoom - 1;
        }
    ...
    function getCurrentExtent(){
        ...
        let mercator = "+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext  +no_defs";
        
        let convertedAlbers;
        let convertedAlbersMax;
        let convertedAlbersMin;
        
        if(SMK.MAP[1].viewer.type == "esri3d")
    	{
        	let bbox = SMK.TYPE.Esri3d.geometry.support.webMercatorUtils.webMercatorToGeographic(SMK.MAP[1].$viewer.view.extent);
        	
        	convertedAlbersMax = (proj4(longlat, mercator,[parseFloat(bbox.xmax), parseFloat(bbox.ymax)]));
            convertedAlbersMin = (proj4(longlat, mercator,[parseFloat(bbox.xmin), parseFloat(bbox.ymin)]));
        }
        else
    	{
            ...
            convertedAlbersMax = (proj4(longlat, mercator, [parseFloat(maxXEastLng), parseFloat(maxYNorthLat)]));
            convertedAlbersMin = (proj4(longlat, mercator, [parseFloat(minXWestLng), parseFloat(minYSouthLat)]));
    	}

        console.log(convertedAlbersMax);
        console.log(convertedAlbersMin);
        
        convertedAlbers = { "maxX": convertedAlbersMax[0], "maxY": convertedAlbersMax[1], "minX": convertedAlbersMin[0], "minY": convertedAlbersMin[1]};
    ...
    function getCurrentBaseMapUrl(){
        ...
        if(SMK.MAP[1].viewer.type == "esri3d")
    	{
        	if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Topo_Map")) 
        		baseMapUrl = "https://services.arcgisonline.com/arcgis/rest/services/World_Topo_Map/MapServer"; 
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Street_Map"))
            	baseMapUrl = "https://services.arcgisonline.com/arcgis/rest/services/World_Street_Map/MapServer";
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Imagery")) 
            	baseMapUrl = "https://services.arcgisonline.com/arcgis/rest/services/World_Imagery/MapServer";
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Ocean_Base")) 
            	baseMapUrl = "https://services.arcgisonline.com/arcgis/rest/services/Ocean/World_Ocean_Base/MapServer";
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("NatGeo_World_Map")) 
            	baseMapUrl = "https://services.arcgisonline.com/ArcGIS/rest/services/NatGeo_World_Map/MapServer";
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Dark_Gray_Base")) 
            	baseMapUrl = "https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer";
            else if (SMK.MAP[1].$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Light_Gray_Base")) 
    ...
    async function sendExportWebMapRequest(jsonData, fileType, template){
        // get the URL
        let url = "https://maps.gov.bc.ca/arcgis/rest/services/Utilities/PrintingTools/GPServer/Export%20Web%20Map%20Task/execute";
        let json;
        ...
        formData.append("outSR", 3857);
        formData.append("processSR", 3857);
