...
calculateZoomScale: function(zoom)
{
    let zoomScale = [];
...
rebuildSMKMAP: async function(mapConfig) {
    ...
    if (smkMAPDIV.nodeName == "BODY"){
        ...
        divNode.setAttribute("style", "height: calc(100vh - 40px);"); // only for custom header
...
////////////////////////////////////////////////////////////////////SMK EXPORT FUNCTIONALITY //////////////////////////////////////////////////////////////////////////////////////////
    ...
    copyFromsmk: function( jsonObjectHolder, smk) {
        ...
        if ( jsonObjectHolder.hasOwnProperty("viewer")  && smk.hasOwnProperty('viewer')){
            ...
            // handle basemap
            let baseMap = "Topographic";
            
            // leaflet viewer only, esri3d basemaps are different
            if(smk.$viewer.currentBasemap)
        	{
            	if ( smk.$viewer.currentBasemap[0]._url.includes("World_Topo_Map")) baseMap = "Topographic"; 
                else if ( smk.$viewer.currentBasemap[0]._url.includes("World_Street_Map")) baseMap = "Streets";
                else if ( smk.$viewer.currentBasemap[0]._url.includes("World_Imagery")) baseMap = "Imagery";
                else if ( smk.$viewer.currentBasemap[0]._url.includes("World_Ocean_Base")) baseMap = "Oceans";
                else if ( smk.$viewer.currentBasemap[0]._url.includes("NatGeo_World_Map")) baseMap = "NationalGeographic";
                else if ( smk.$viewer.currentBasemap[0]._url.includes("World_Dark_Gray_Base")) baseMap = "DarkGray";
                else if ( smk.$viewer.currentBasemap[0]._url.includes("World_Light_Gray_Base")) baseMap = "Gray";
        	}
            else // esri3d viewer
        	{
            	if ( smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Topo_Map")) baseMap = "Topographic"; 
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Street_Map")) baseMap = "Streets";
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Imagery")) baseMap = "Imagery";
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Ocean_Base")) baseMap = "Oceans";
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("NatGeo_World_Map")) baseMap = "NationalGeographic";
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Dark_Gray_Base")) baseMap = "DarkGray";
                else if (smk.$viewer.view.basemapView.baseLayerViews.items[0].layer.attributionDataUrl.includes("World_Light_Gray_Base")) baseMap = "Gray";
        	}
    ...
    combineGeometryCollectionsAndPointsAndMultiPoints: function( tempJsonObjectHolder, jsonObjectHolder){
        for ( let maybeGeoCollection in jsonObjectHolder.drawings){
            ...
            if (jsonObjectHolder.drawings[maybeGeoCollection] && jsonObjectHolder.drawings[maybeGeoCollection].type == "Feature" || jsonObjectHolder.drawings[maybeGeoCollection].geometry.type == "GeometryCollection" ){
