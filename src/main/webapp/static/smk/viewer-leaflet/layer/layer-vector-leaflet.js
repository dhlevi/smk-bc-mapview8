include.module( 'layer-leaflet.layer-vector-leaflet-js', [ 'layer.layer-vector-js', 'turf' ], function () {
    ...
    SMK.TYPE.Layer[ 'vector' ][ 'leaflet' ].createClusterLayer = function (sourceData, ly)
    {
    	var layer = new L.geoJson( null, {
            pointToLayer: function ( geojson, latlng ) {
                return markerForStyle( SMK.MAP[1].$viewer, latlng, ly.style )
            },
            onEachFeature: function ( feature, layer ) {
                if ( layer.setStyle )
                    layer.setStyle( convertStyle( ly.style, feature.geometry.type ) )
            },
            renderer: L.svg(),
            interactive: false
        } );
    ...
    SMK.TYPE.Layer[ 'vector' ][ 'leaflet' ].create = function ( layers, zIndex ) {
        ...
        // If this is a WFS layer, we don't want to resolve the URL yet, unless it's within the viewable range.
        // so set the default bounds to something really small, or set return limit to 0?
        var dataUrl = layers[ 0 ].config.dataUrl;
        if(layers[0].config.isWfs) dataUrl += "&maxFeatures=0";
